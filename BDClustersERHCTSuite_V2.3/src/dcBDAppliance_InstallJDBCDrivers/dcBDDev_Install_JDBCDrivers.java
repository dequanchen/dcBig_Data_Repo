...
public class dcBDDev_Install_JDBCDrivers {
	...
	public static void main(String[] args) {		
		...
		System.out.println("*** clusterENNumber is " + clusterENNumber);
		...
		for (int i = 0; i < clusterENNumber; i++){ //bdClusterEntryNodeList.size()..1
			...
			for (String jdbcJarFileName: jdbcJarFileNameList){
				...
				if (exitVal ==0) {
					System.out.println("*** Success - #(" + countJarFile + ") " + jdbcJarFileName + " has been installed on '"  + bdClusterName + "' entry node - " + tempENName);
				} else {
