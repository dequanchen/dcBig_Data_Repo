...
public class A2_dcTestMapReduce_WordCounting {
	...
	public static void run() throws Exception {
		...
		//System.out.print("*** loginUserName is: " + loginUserName);
		...
		for (int i = clusterENNumber_Start; i < clusterENNumber; i++){ //bdClusterEntryNodeList.size()..1..clusterENNumber	
			...
			ULServerCommandFactory bdENCmdFactory = aBDNode.getBdENCommFactory(); //.getBdENCmdFactory();			
			...
			//}			
			//System.out.print("*** loginUserName is: " + loginUserName);
			System.out.print("*** loginUserName is: root!!!");
			HdfsUtil.safelyCreateAFolder_OnEntryNodeLocal_OnBDCluster(enServerScriptFileDirectory, bdENCmdFactory);
			System.out.print("*** On '" + tempENName + "'server, created enServerScriptFileDirectory: " + enServerScriptFileDirectory);
						
						
						
			//(1) Move MapReduce test data file and jar file into Linux local folder for testing
			int exitVal1 =  HdfsUtil.copyFile_FromWindowsLocal_ToEntryNodeLocal_OnBDCluster(localMapReduceTestDataFileName, 
					bdClusterUATestResultsParentFolder, enServerScriptFileDirectory, bdENCmdFactory);			
			
			int exitVal2 = HdfsUtil.copyFile_FromWindowsLocal_ToEntryNodeLocal_OnBDCluster(localMapReduceJarFileName, 
					bdClusterUATestResultsParentFolder, enServerScriptFileDirectory, bdENCmdFactory);
			...
			String runMapReduceWrodCountJarCmd = "/usr/bin/hadoop jar " + enServerMapReduceJarFilePathAndName +  " mapreduce.WordCount " + mapReduceTestInputFolder + " " + mapReduceTestOutputFolder ;
			
						
			//sb.append("chown hdfs:hdfs " + enServerScriptFileDirectory + ";\n");
			//sb.append("sudo su - hdfs;\n");
			sb.append("chown -R " + "hdfs:hadoop " + enServerScriptFileDirectory + ";\n"); //loginUser:users
			...
			sb.append("hadoop fs -chown -R "  + "hdfs:hdfs " + mapReduceTestFolderName + "; \n");	 //loginUserName:bdadmin		
		    ...
		    sb.append(runMapReduceWrodCountJarCmd + ";\n");
			...
			HdfsUtil.runScriptFile_OnBDCluster(localMapReduceWordCountTestingScriptFilePathAndName, 
