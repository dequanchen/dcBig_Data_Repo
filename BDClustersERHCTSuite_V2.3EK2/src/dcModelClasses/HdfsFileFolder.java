...
public class HdfsFileFolder {
	private String bdClusterName = ""; //BDProd2..BDTest2..BDDev1..BDSbx
...
////////////////////////////////////////////////////////////////////	
	public  static ArrayList<String> getMaximumTargetLevelAllSubFolderListInAHdfsFolder (String aBdClusterName, String hdfsParentFolderPath, int targetDirLevel ) {
		...
		allLevelFolderList = getAllTargetDirLevelSubFolderListInAHdfsFolder (currHadoopFS, hdfsParentFolderPath, allLevelFolderList, 0, targetDirLevel);
	...
	private  static ArrayList<String> getAllTargetDirLevelSubFolderListInAHdfsFolder (FileSystem currHadoopFS, String hdfsParentFolderPath, ArrayList<String> allFolderList, int currDirLevel, int targetDirLevel) {
		try {
				...
				if (status[i].isDirectory()){
					...
					if (currHdfsFolderPathAndName.contains("hdfs://") && currHdfsFolderPathAndName.contains(":8020")){
						...
						if (currDirLevel < targetDirLevel ){
							getAllTargetDirLevelSubFolderListInAHdfsFolder (currHadoopFS, currHdfsFolderPathAndName, allFolderList, currDirLevel, targetDirLevel );
						}//end inner if								
					}//end middle if					
				} //end outer if		
			}//end for
...
////////////////////////////////////////////////////////////////////////////////////
	public static ArrayList<String> getCurrentDirLevel1SimplifiedSubDirList (FileSystem currHadoopFS, String aHdfsFolderPathName){
			...
			for (int i = 0; i < status.length; i++) {//0..status.length								
				...
				if (status[i].isDirectory()){ //.isDir()){
					...
					if (currHdfsFolderPathAndName.contains("hdfs://") && currHdfsFolderPathAndName.contains(":8020")){
						...
						String simpleSubFolderFullPathName = folderNameSplit[1].trim();						
						allHdfsSubFolderList.add(simpleSubFolderFullPathName);						
					}
	...
	public static ArrayList<String> getCurrentDirLevel1FullSubDirList (FileSystem currHadoopFS, String aHdfsFolderPathName){
		ArrayList<String> allHdfsSubFolderList = new ArrayList<String> ();
		try {
...
//////////	
	...
	public  static ArrayList<String> getAllHdfsSubFoldersInAParentFolder (String aBdClusterName, String hdfsParentFolderPath){
		...
		allSubFolderList = getAllHdfsFoldersInAParentFolder (currHadoopFS, hdfsParentFolderPath, allSubFolderList);
		return allSubFolderList;
	}//end getAllHdfsSubFoldersInAParentFolder
			
	private  static ArrayList<String> getAllHdfsFoldersInAParentFolder (FileSystem currHadoopFS, String hdfsParentFolderPath, ArrayList<String> allSubFolderList) {
		try {
			//BdCluster currBdCluster = new BdCluster(aBdClusterName);
			//FileSystem currHadoopFS  = currBdCluster.getHadoopFS();
			FileStatus[] status = currHadoopFS.listStatus(new Path(hdfsParentFolderPath));				
			...
			for (int i = 0; i < status.length; i++) {//0..status.length								
				...
				if (status[i].isDirectory()){
					//folderCountTotal++;
					currHdfsFolderPathAndName = status[i].getPath().toString();
					if (currHdfsFolderPathAndName.contains("hdfs://") && currHdfsFolderPathAndName.contains(":8020")){ //hdfs://hdpr0...hdfs://...hdfs://hdp00
						String[] folderNameSplit = currHdfsFolderPathAndName.split(":8020");					
						currHdfsFolderPathAndName = folderNameSplit[1];
						//System.out.println("  *** HDFS Sub-Folder:  " + currHdfsFolderPathAndName + ";");
						allSubFolderList.add(currHdfsFolderPathAndName);
						//System.out.println("\n*** # (" + folderCountTotal + ") Current HDFS Folder: \n ---" + currHdfsFolderPathAndName + ";");
						getAllHdfsFoldersInAParentFolder (currHadoopFS, currHdfsFolderPathAndName, allSubFolderList);
					}					
				} 
			}//end for
		} catch (IOException e) {			
...
/////////////////////////////////////////////////////////////////////////////////////	
	...
	private ArrayList<String> getHdfsFilesInAFolder (String hdfsFileOrFolderPath, String fileNameSelector) {
		...
		if (fileNameSelector.equalsIgnoreCase("") || fileNameSelector.equalsIgnoreCase("all")){
			this.getAllHdfsFilesInAFolder (currHadoopFS, hdfsFileOrFolderPath, selectedFileList);
		} else {
			this.getSelectedAllHdfsFilesInAFolder (currHadoopFS, hdfsFileOrFolderPath, selectedFileList,  fileNameSelector);
		}
	...
	private void getAllHdfsFilesInAFolder (FileSystem currHadoopFS, String hdfsFileOrFolderPath, ArrayList<String> allFilesList) {
		//int fileCount = 0;
		try {
			//BdCluster currBdCluster = new BdCluster(this.bdClusterName);
			//FileSystem currHadoopFS  = currBdCluster.getHadoopFS();			
			...
			for (int i = 0; i < status.length; i++) {//0..status.length								
				...
				if (!status[i].isDirectory()){ //.isDir()){
					...
					getAllHdfsFilesInAFolder(currHadoopFS, subHdfsFileOrFolderPath,allFilesList);
				}
			}//end for
...
//			}//end while			
	...
	private void getSelectedAllHdfsFilesInAFolder (FileSystem currHadoopFS, String hdfsFileOrFolderPath, ArrayList<String> allFilesList, String fileNameSelector) {
		//int fileCount = 0;
		try {
			//FileSystem fs = FileSystem.get(new Configuration());
			//FileStatus[] status = fs.listStatus(new Path(hdfsFileOrFolderPath));			
			...
			for (int i = 0; i < status.length; i++) {//0..status.length	
				...
				if (!status[i].isDirectory()){ //.isDir()){
					...
					String subHdfsFileOrFolderPath = status[i].getPath().toString().replace("file:/", "");
					//System.out.println("\n*2* : subHdfsFileOrFolderPath: " + subHdfsFileOrFolderPath + "");
					getSelectedAllHdfsFilesInAFolder(currHadoopFS, subHdfsFileOrFolderPath,allFilesList, fileNameSelector);
				}
			}//end for
